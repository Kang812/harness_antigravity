...
**핵심 원칙:**
1. 에이전트 정의(`.agents/`)와 스킬(`.agents/skills/`)을 생성한다.
2. **에이전트 팀(다중 서브에이전트 병렬 호출)을 기본 실행 모드로 사용한다.**
3. **README.md(또는 설명서)에 하네스 포인터를 등록한다.** — 새 세션에서 오케스트레이터 스킬이 트리거되도록 최소한의 포인터(트리거 규칙 + 변경 이력)만 기록한다.
4. **하네스는 고정물이 아니라 진화하는 시스템이다.** — 매 실행 후 피드백을 반영하고, 에이전트·스킬·설명서를 지속 갱신한다.
...
1. `프로젝트/.agents/`, `프로젝트/.skills/`, `프로젝트/README.md`를 읽는다
...
| 모드 | 언제 사용 | 특성 |
|------|----------|------|
| **에이전트 팀** (기본) | 2명 이상 협업, 실시간 조율·피드백 교환이 필요, 중간 산출물 상호 참조 | `invoke_subagent` 다중 호출 + `send_message` + 파일 기반 조율 |
| **서브 에이전트** (대안) | 단일 에이전트 작업, 결과만 메인에 반환하면 충분, 팀 통신 오버헤드가 과할 때 | `invoke_subagent` 단일 호출, `Workspace: 'branch'`로 병렬 |
| **하이브리드** | Phase마다 특성이 다를 때 — 예: 병렬 수집(서브) → 합의 기반 통합(팀) | Phase 단위로 팀/서브를 섞어 구성 |

**의사결정 순서:**
1. 먼저 에이전트 팀으로 설계 가능한지 검토한다 — 2명 이상이면 기본값 (Workspace를 공유(share)하거나 메시지 기반으로 조율)
...
신규 에이전트 생성 전, `프로젝트/.agents/`의 기존 에이전트와 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 역할이 겹치는 에이전트가 다른 이름으로 누적되기 쉽다.

> 중복 분류 기준과 재사용 설계는 `references/agent-design-patterns.md`의 "에이전트 재사용 설계" 참조.

**모든 에이전트는 반드시 `프로젝트/.agents/{name}.md` 파일로 정의한다.** 에이전트 정의 파일 없이 에이전트의 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
- 에이전트 정의가 파일로 존재해야 다음 세션에서 재사용 가능
- `define_subagent` 도구로 동일하게 불러와 사용하기 위함
- 팀 통신 프로토콜이 명시되어야 에이전트 간 협업 품질 보장
- 하네스의 핵심 가치는 에이전트(누가)와 스킬(어떻게)의 분리

빌트인 타입(`general-purpose`, `Explore`, `Plan`)을 가이드라인으로 활용하더라도 에이전트 정의 파일은 생성한다. 에이전트 정의 파일에는 역할·원칙·프로토콜을 담고, 실제 구동 시에는 `define_subagent` 도구로 정의하여 호출한다.

**에이전트 최적화:** Antigravity의 `invoke_subagent` 호출 시, 각 서브에이전트가 최적의 기술적 권한(write tools, subagent tools 등)을 가지도록 `define_subagent` 단계에서 권한 옵션(`enable_write_tools`, `enable_subagent_tools`)을 올바르게 설정한다.

**팀 재구성:** Phase 간에 서브에이전트를 해체하고(필요시 `manage_subagents`의 kill 액션 사용) 새 서브에이전트를 호출하여 새 팀을 구성할 수 있다. 파이프라인 패턴처럼 Phase별로 다른 전문가 조합이 필요하면, 이전 에이전트들의 산출물을 공유 파일 영역(`Workspace: 'share'`)에 저장한 뒤 새 에이전트를 스폰한다.
...
**QA 에이전트 포함 시 필수 사항:**
- QA 에이전트는 `enable_write_tools: true` 및 `enable_subagent_tools: true` 옵션이 켜진 타입을 사용하라 (읽기 전용이 아니어야 스크립트 실행 및 파일 검증이 가능)
...
**에이전트 팀 패턴 (기본):**
오케스트레이터가 `define_subagent`로 에이전트를 정의하고, `invoke_subagent`로 서브에이전트들을 스폰한다. 이때 `Workspace: "share"`를 지정하여 공통 디렉토리를 사용한다. 팀원들은 `send_message`로 직접 통신하며 자체 조율하고, 공유 파일(`_workspace/tasks.json` 등)을 통해 업무 상태를 확인한다.

```
[오케스트레이터/리더]
    ├── define_subagent (타입 정의)
    ├── invoke_subagent (Workspace: "share" 다중 호출)
    ├── 태스크 파일 생성 (_workspace/tasks.json)
    ├── 팀원들이 자체 조율 (send_message & 파일 갱신)
    ├── 결과 수집 및 종합
    └── 서브에이전트 정리 (manage_subagents: kill)
```

**서브 에이전트 패턴 (대안):**
오케스트레이터가 `invoke_subagent` 도구로 서브 에이전트를 독립적으로 호출한다. 병렬 실행은 `invoke_subagent` 내 `Subagents` 배열에 여러 건을 명시하고 `Workspace: 'branch'`로 격리하거나 독립 처리하며, 결과는 메인에게 반환된다. 팀 통신이 불필요하고 오버헤드를 줄이고 싶을 때 사용.

```
[오케스트레이터]
    ├── invoke_subagent(Subagents: [agent-1, agent-2])
    ├── 결과 대기 및 수집 (반환값/파일)
    └── 통합 산출물 생성
```
...
- **병렬 수집(서브) → 합의 통합(팀)**: Phase 2에서 서브 에이전트로 독립 자료를 병렬 수집 → Phase 3에서 팀(공유 워크스페이스)을 만들어 토론·합의 기반 통합
- **팀 생성(팀) → 검증(서브)**: Phase 2에서 팀이 초안 생성 → Phase 3에서 단일 서브 에이전트가 독립 검증
- **Phase 간 팀 재구성**: 각 Phase마다 불필요한 서브에이전트를 정리(`manage_subagents: kill`)하고 새 에이전트들을 `invoke_subagent`로 스폰
...
| **태스크 기반** | 공유 파일(`tasks.json`) 갱신으로 작업 상태 공유 | 팀 | 진행상황 추적, 의존 관계 관리, 작업 자체 요청 |
| **파일 기반** | 약속된 경로에 파일을 쓰고 읽음 | 팀 + 서브 | 대용량 데이터, 구조화된 산출물, 감사 추적 필요 |
| **반환값 기반** | `invoke_subagent` 도구의 반환 메시지 | 서브 | 서브 에이전트 결과를 메인이 직접 수집 |

**권장 조합 (팀 모드):** 태스크 기반(파일 조율) + 파일 기반(산출물) + 메시지 기반(실시간 소통)
...
#### 5-4. README.md 하네스 포인터 등록

하네스 구성 완료 후, 프로젝트의 `README.md`(또는 에이전트 지침서)에 최소한의 포인터를 등록한다. 프로젝트 안내 문서는 세션 시작 시 로딩되므로, 하네스 존재와 트리거 규칙만 기록하면 오케스트레이터 스킬이 나머지를 처리한다.
...
**설명서에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.agents/`, `.agents/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. 설명서에는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
- 에이전트 간 참조 일관성 확인
- 커맨드가 생성되지 않았는지 확인 (예: `.agents/commands/` 등)

#### 6-2. 실행 모드별 검증

- **에이전트 팀**: `invoke_subagent` 호출 구조, 팀원 간 메시지 통신 경로, 작업 의존성(공유 파일), 팀 크기 적정성 확인
...
모든 변경은 README.md(또는 설명서)의 **변경 이력** 테이블에 기록한다 (Phase 5-4 템플릿의 "변경 이력" 섹션과 동일 테이블):
...
**Step 1: 현황 감사**
- `.agents/` 파일 목록과 오케스트레이터 스킬의 에이전트 구성 비교 → 불일치 목록 생성
...
- README.md와 실제 파일의 일치 여부 최종 확인

## 산출물 체크리스트

생성 완료 후 확인:

- [ ] `프로젝트/.agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
...
- [ ] 모든 Agent 호출에 `invoke_subagent` 도구 파라미터 규격 준수
- [ ] 신규 에이전트 생성 전 기존 에이전트 중복 검토 완료 (Phase 3-0)
- [ ] 신규 스킬 생성 전 기존 스킬 중복 검토 완료 (Phase 4-0)
- [ ] `.agents/commands/` 등 커맨드 경로 — 아무것도 생성하지 않음
- [ ] 기존 에이전트/스킬과 충돌 없음
